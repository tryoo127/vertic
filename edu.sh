...
wget -q -O /usr/local/bin/edussh-nontls https://raw.githubusercontent.com/tryoo127/vertic/main/proxy-template.py
chmod +x /usr/local/bin/edussh-nontls

# Installing Service
cat > /etc/systemd/system/edussh-nontls.service << END
[Unit]
Description=Ssh Websocket By XoolVPN
...
wget -q -O /usr/local/bin/edu-ovpn https://raw.githubusercontent.com/tryoo127/vertic/main/SSHOVPN/edu-op.py
chmod +x /usr/local/bin/edu-ovpn

# Installing Service Ovpn Websocket
cat > /etc/systemd/system/edu-ovpn.service << END
[Unit]
Description=Ovpn Websocket By XoolVPN
Documentation=https://google.com
After=network.target nss-lookup.target
